...
import { clipDescription, extractDescription, resolveImageInput } from './vision-core.ts'
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      const payload = (await response.json()) as Parameters<typeof extractDescription>[0]
      const description = extractDescription(payload)
      return { description: clipDescription(description) }
    },
    presentCall,
    presentResult,
  }))
